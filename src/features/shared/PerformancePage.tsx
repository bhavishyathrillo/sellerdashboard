...
export default function PerformancePage({ session }: Props) {
  ...
  const isManager = isL1 || isL2 || ['ADMIN', 'MODERATOR'].includes(session.role)

  const l1TeamUrl = isL1 ? `/api/seller/team-performance?email=${encodeURIComponent(session.email)}&role=${session.role}` : null
  const overviewUrl = !isL1 ? `/api/seller/overview?email=${encodeURIComponent(session.email)}` : null
  const managerTeamUrl = (!isL1 && isManager) ? `/api/seller/team-performance?email=${encodeURIComponent(session.email)}&role=${session.role}` : null

  const { data: teamDataL1, loading: l1TeamLoading } = useCachedFetch(l1TeamUrl)
  const { data: overviewData, loading: overviewLoading } = useCachedFetch(overviewUrl)
  const { data: teamDataManager, loading: managerTeamLoading } = useCachedFetch(managerTeamUrl)

  useEffect(() => {
    if (isL1) {
      if (teamDataL1) setTeamData(teamDataL1)
      if (teamDataL1 || !l1TeamLoading) setLoading(false)
    } else {
      if (overviewData) setData(overviewData.myMonthly || overviewData)
      if (isManager && teamDataManager) setTeamData(teamDataManager)
      if ((overviewData || !overviewLoading) && (!isManager || teamDataManager || !managerTeamLoading)) {
        setLoading(false)
      }
    }
  }, [isL1, isManager, teamDataL1, l1TeamLoading, overviewData, overviewLoading, teamDataManager, managerTeamLoading])
