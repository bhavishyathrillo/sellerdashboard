...
export default function RewardsPage({ session }: Props) {
  ...
  if (session.role === 'L1') {
    ...
    const { data: fetchedL1, loading: isFetchingL1 } = useCachedFetch(`/api/seller/tl/rewards?email=${encodeURIComponent(session.email)}`)

    useEffect(() => {
      if (fetchedL1) {
        setL1Rewards(fetchedL1)
        setL1Loading(false)
      } else if (!isFetchingL1) {
        setL1Loading(false)
      }
  ...
  // ========== NON-L1 ==========
  const { data: fetchedRewards, loading: isFetchingRewards } = useCachedFetch(`/api/seller/seller/rewards?email=${encodeURIComponent(session.email)}`)

  useEffect(() => {
    if (fetchedRewards) {
      setRewards(fetchedRewards)
      setLoading(false)
    } else if (!isFetchingRewards) {
      setLoading(false)
    }
  }, [fetchedRewards, isFetchingRewards])

  async function loadRewards() {
    try {
      const res = await fetch(`/api/seller/seller/rewards?email=${encodeURIComponent(session.email)}&t=${Date.now()}`)
