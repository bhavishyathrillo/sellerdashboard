...
type HourRow = { hour: string; count: number; cumLeads: number; cumPct: number };
...
export default function HourlyViewModal(props: any) {
  ...
  let cumLeads = 0;
  const hourRows: HourRow[] = (HOUR_SLOTS as string[]).map((hour: string) => {
  ...
  const maxCount = Math.max(...hourRows.map((r: HourRow) => r.count), 1);
  return (
    <div style={{ marginTop: '14px', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '12px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '4px', height: '60px', marginBottom: '4px' }}>
        {hourRows.map(({ hour, count, cumLeads: cum, cumPct }: HourRow) => {
      ...
      <div style={{ display: 'flex', gap: '4px' }}>
        {(HOUR_SLOTS as string[]).map((h: string) => {
