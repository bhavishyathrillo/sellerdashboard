...
export async function GET(req: Request) {
  const fetchAll = async (builder: any) => {
    ...
    while (hasMore) {
      ...
      for (let i = 0; i < maxConcurrency; i++) {
        promises.push(builder.range(from, from + step - 1))
        from += step
      }
      const chunks = await Promise.all(promises)
      for (const { data, error } of chunks) {
        if (error) { console.error(error); lastError = error; hasMore = false; break }
  ...
  try {
    ...
    const { data: allSellers, error: sellersError } = allSellersRes
    if (sellersError) {
