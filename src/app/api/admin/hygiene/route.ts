...
export async function GET() {
  try {
    ...
    const effPromise = (async () => {
      ...
      while (hasMore) { let query = supabase.from('efficiency_2').select('id, seller_email, call_date, total_dials, total_answered_mins, unique_enquiries_called').gte('call_date', dateFrom).lte('call_date', dateTo).order('id', { ascending: true }).limit(pageSize); if (lastId > 0) query = query.gt('id', lastId); const { data: chunk, error } = await query; if (error) throw error; if (!chunk || chunk.length === 0) hasMore = false; else { allEfficiency = allEfficiency.concat(chunk); lastId = chunk[chunk.length - 1].id; if (chunk.length < pageSize) hasMore = false } }
