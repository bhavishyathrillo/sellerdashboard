...
export async function GET(req: Request) {
  try {
    ...
    const tlName = searchParams.get('name')
    if (!tlName) {
      return NextResponse.json({ error: 'Name parameter is required' }, { status: 400 })
    }

    const decodedName = tlName.toLowerCase().trim()

    // 1. Fetch all sellers under this TL from srs_raw
    const { data: rawSellers, error: rawError } = await supabase
      .from('srs_raw')
      .select('seller_email, l1_name, l2_name, current_seller_flag')
      .ilike('l2_name', `%${decodedName}%`)
    ...
    if (!rawSellers || rawSellers.length === 0) {
      return NextResponse.json({
        ...
        totalSellers: 0,
        totalL1: 0,
        message: 'No sellers found for this TL in srs_raw'
      })
    }
    ...
    // Build data for the TL (we wrap it exactly like CM data so frontend can reuse logic)
    ...
    team.forEach((r: any) => {
      const regionName = r.region?.trim() || 'Unassigned Region'
      if (!regionMap.has(regionName)) {
        regionMap.set(regionName, [])
      }
      regionMap.get(regionName)!.push(r)
    })

    const l2Groups = Array.from(regionMap.entries()).map(([regionName, regSellers]) => {
      const regGoal = regSellers.reduce((s: number, r: any) => s + (Number(r.bottomline_goal) || 0), 0)
      const regAch = regSellers.reduce((s: number, r: any) => s + (Number(r.bl_actual_splits) || 0), 0)

      return {
        l2_name: regionName,
        l2_email: regionName, // Mock email
        l2_goal: regGoal,
        l2_achieved: regAch,
        l2_pct: regGoal > 0 ? (regAch / regGoal) * 100 : 0,
        seller_count: regSellers.length,
        sellers: regSellers.map((s: any) => {
          return {
    ...
    // TL acts as L1 for the frontend structure
    l1Data.push({
      l1_email: decodedName,
      l1_name: decodedName,
    ...
    perfRows.forEach((row: any) => {
      const tlName = decodedName
      const regionName = row.region?.trim() || 'Unassigned Region'
      ...
      if (sEmail) {
        ...
        if (!flagData.has(tlName)) {
           flagData.set(tlName, { totalCounts: {}, l2Groups: new Map() })
        }
        
        const tlData = flagData.get(tlName)!
        
        tlData.totalCounts[flag] = (tlData.totalCounts[flag] || 0) + 1
        
        if (!tlData.l2Groups.has(regionName)) {
          tlData.l2Groups.set(regionName, {})
        }
        const regionCounts = tlData.l2Groups.get(regionName)!
        regionCounts[flag] = (regionCounts[flag] || 0) + 1
      }
    })

    const flagTable = {
      columns: Array.from(allFlags).sort(),
      rows: Array.from(flagData.entries()).map(([tlName, data]) => ({
        cmName: tlName, // Keep name as cmName for frontend compatibility
        counts: data.totalCounts,
        l2Rows: Array.from(data.l2Groups.entries()).map(([regionName, counts]) => ({
          l2Name: regionName,
          counts
