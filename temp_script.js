...
// ═══════════════ DASHBOARD ═══════════════
function loadDashboard(){api('dashboard', {from: globalFilters.from||null, to: globalFilters.to||null}).then(function(data){if(!data||data.error)return;dashData=data;_initCycle();renderAllViews();}).catch(function(err){console.error(err);});}
function refreshDashboard(){document.getElementById('loading-screen').style.display='flex';api('dashboard', {from: globalFilters.from||null, to: globalFilters.to||null}).then(function(data){if(!data||data.error){document.getElementById('loading-screen').style.display='none';return;}dashData=data;document.getElementById('loading-screen').style.display='none';renderAllViews();}).catch(function(err){console.error(err);document.getElementById('loading-screen').style.display='none';});}
...
function _applyCycleDates(){globalFilters.from='';globalFilters.to='';rawCache={};mhlCache={};}
...
// ═══════════════ DATE FILTER ═══════════════
function buildDateUI(view){var df=globalFilters;var r=_cycleRange(_selectedCycle);var minDate='2026-07-09';var cMin=(r&&r.from<minDate)?minDate:(r?r.from:minDate);var mm=' min="'+cMin+'"'+(r?' max="'+r.to+'"':'');return'<div class="date-range-wrap"><span class="date-range-label">From</span><input type="date" class="date-input" id="'+view+'-from" value="'+(df.from||'')+'"'+mm+' onchange="onDateChange(\''+view+'\')"><span class="date-sep">→</span><span class="date-range-label">To</span><input type="date" class="date-input" id="'+view+'-to" value="'+(df.to||'')+'"'+mm+' onchange="onDateChange(\''+view+'\')"><button class="date-clear" onclick="clearDate(\''+view+'\')">✕</button></div>'+((df.from||df.to)?'<span class="date-active-badge">📅 filtered</span>':'');}
function onDateChange(view){var f=document.getElementById(view+'-from'),t=document.getElementById(view+'-to');if(!f||!t)return;globalFilters.from=f.value;globalFilters.to=t.value;saveState();rawCache={};mhlCache={};RC_DATA=null;refreshDashboard();if(view==='rc'||document.getElementById('page-report').style.display==='block'){RC_DATA=null;rcLoadReportCard();}}
function clearDate(view){globalFilters.from='';globalFilters.to='';saveState();rawCache={};mhlCache={};RC_DATA=null;refreshDashboard();if(view==='rc'||document.getElementById('page-report').style.display==='block'){RC_DATA=null;rcLoadReportCard();}}

// ═══════════════ RAW & MHL FETCH ═══════════════
function fetchRaw(sellers,view,cb){var emails=sellers.map(function(s){return s.email;});var df=globalFilters;var r=_cycleRange(_selectedCycle);var from=df.from||(r?r.from:null);var to=df.to||(r?r.to:null);var key=emails.slice().sort().join('|')+'||'+(from||'')+'|'+(to||'');if(rawCache[key]){cb(rawCache[key]);return;}api('raw',{emails:emails.join(','),from:from,to:to},'POST').then(function(res){if(res&&res.success)rawCache[key]=res;cb(res||{});}).catch(function(){cb({});});}
function fetchMHL(sellers,view,cb){var emails=sellers.map(function(s){return s.email;});var df=globalFilters;var r=_cycleRange(_selectedCycle);var from=df.from||(r?r.from:null);var to=df.to||(r?r.to:null);var key='mhl|'+emails.slice().sort().join('|')+'||'+(from||'')+'|'+(to||'');if(mhlCache[key]){cb(mhlCache[key]);return;}api('mhl',{emails:emails.join(','),from:from,to:to},'POST').then(function(res){if(res&&res.success)mhlCache[key]=res;cb(res||{});}).catch(function(){cb({});});}
