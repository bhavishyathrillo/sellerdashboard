...
export async function GET() {
  try {
    ...
    // Get all pipeline submissions
    const { data: submissions } = await supabase.from('pipeline_submissions').select('*').order('date', { ascending: false }).limit(500)
    ...
    for (const [, l1] of l1Map) {
      ...
      for (const [, l2] of l1.l2Map) {
        ...
        const l2Subs = (submissions || []).filter((s: any) => 
          sellerEmails.includes(s.seller_email?.toLowerCase().trim())
        )
        const l2Green = l2Subs.filter((s: any) => s.status === 'GREEN').length

        if (sellerEmails.length > 0) {
          l2Groups.push({
            l2_name: l2.l2_name, l2_email: l2.l2_email,
            seller_count: sellerEmails.length,
            submissions: l2Subs
          })
          l2Count++
          totalSubs += l2Subs.length
          greenCount += l2Green
        }
      }

      const allL1Subs = l2Groups.flatMap((g: any) => g.submissions)

      l1Data.push({
        l1_name: l1.l1_name, l1_email: l1.l1_email,
        total_submissions: totalSubs, green_count: greenCount,
        l2_count: l2Count,
        submissions: allL1Subs,
        l2_groups: l2Groups
      })
    }
