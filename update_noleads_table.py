...
old_modal_comp = """function NoLeadsModal({ sellers, onClose }: { sellers: any[], onClose: () => void }) {
  // Group by TL and CM
...
new_modal_comp = """function NoLeadsModal({ sellers, onClose }: { sellers: any[], onClose: () => void }) {
  return (
    <div className="la-modal-overlay" onClick={onClose}>
      <div className="la-modal-card" onClick={e => e.stopPropagation()} style={{ width: '850px' }}>
        <button className="la-modal-close" onClick={onClose}>✕</button>
...
if old_modal_comp in text:
    text = text.replace(old_modal_comp, new_modal_comp)
else:
    print("WARNING: Could not find old modal comp!")
