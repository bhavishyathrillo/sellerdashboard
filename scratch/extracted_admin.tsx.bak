      ...
      <table className="la-table">
        ...
        <tbody>
          {hierarchy.map(cat => {
            ...
            const present = allSellers.filter((s: any) => !s.isAbsent).length
            const late = allSellers.filter((s: any) => s.isLateLogin).length
            const absent = allSellers.filter((s: any) => s.isAbsent).length
            ...
            return (
              <React.Fragment key={catKey}>
                <tr className="la-cat-row" onClick={() => setExpandedCat(expandedCat === catKey ? null : catKey)}>
                  ...
                  <td>{present}</td>
                  <td style={{ color: late > 0 ? '#F59E0B' : '#22C55E' }}>{late}</td>
                  <td style={{ color: absent > 0 ? '#EF4444' : '#22C55E' }}>{absent}</td>
                </tr>
                {expandedCat === catKey && cat.tls.map((tl: any) => {
                  const tlLate = tl.sellers.filter((s: any) => s.isLateLogin).length
                  const tlAbsent = tl.sellers.filter((s: any) => s.isAbsent).length
                  const tlPresent = tl.sellers.filter((s: any) => !s.isAbsent).length
                  ...
                  return (
                    <React.Fragment key={tlKey}>
                      <tr className="la-tl-row" onClick={() => setExpandedTl(expandedTl === tlKey ? null : tlKey)}>
                        ...
                        <td>{tlPresent}</td>
                        <td style={{ color: tlLate > 0 ? '#F59E0B' : '#22C55E' }}>{tlLate}</td>
                        <td style={{ color: tlAbsent > 0 ? '#EF4444' : '#22C55E' }}>{tlAbsent}</td>
                      </tr>
                      {expandedTl === tlKey && tl.sellers.map((s: any) => (
                        <tr key={s.seller_email} className="la-seller-row">
                          ...
                          <td>—</td>
                          <td>{s.isAbsent ? <span className="la-flag la-flag-absent">Absent</span> : formatTime(s.loginTime)}</td>
                          <td>{s.isLateLogin ? <span className="la-flag la-flag-late">Late</span> : s.isAbsent ? '—' : <span className="la-flag la-flag-ok">On Time</span>}</td>
                          <td>{s.isAbsent ? <span className="la-flag la-flag-absent">Yes</span> : '—'}</td>
                        </tr>
                      ))}
                    </React.Fragment>
                  )
