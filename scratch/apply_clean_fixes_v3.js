...
`;
  ...
  const safeInject = (targetStr, injectStr) => {
    ...
    if (startIdx !== -1) {
      const insertionIdx = startIdx + targetStr.length;
      code = code.substring(0, insertionIdx) + "\\n" + injectStr + code.substring(insertionIdx);
    }
  };
  ...
  code = code.replace(/\\n          <div/g, '\\n          <div'); // wait no, just don't escape it in string concatenation
  // I did `"\\n"` which is `\n` literal. I should have used `\n` directly.
  code = code.replace(/\\n/g, '\n');
