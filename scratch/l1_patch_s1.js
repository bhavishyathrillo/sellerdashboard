...
// Content 
// Since S1 is complex with processedGroups, let's replace the whole tableWrap block.
const s1Start = `<div className={styles.tableWrap}>`;
const s1Cards = `{s1View === 'table' ? (
          <div className={styles.tableWrap}>`;

code = code.replace(s1Start, s1Cards);

const s1End = `            </table>
          </div>
        </div>`;
const s1CardsEnd = `            </table>
          </div>
          ) : (
...
// WARNING: s1End appears multiple times maybe? In S1 block it should only appear once.
// Let's use exact replace just for S1
const indexS1 = code.indexOf(s1End, code.indexOf('activeSectionModal === \'s1\''));
if(indexS1 !== -1) {
  code = code.substring(0, indexS1) + s1CardsEnd + code.substring(indexS1 + s1End.length);
}
