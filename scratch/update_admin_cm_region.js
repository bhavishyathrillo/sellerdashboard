...
export default function AdminOverviewPage() {
  ...
  const processedData = l1Data.map((l1: any) => {
    ...
    // Group all sellers under this CM by region directly
    ...
    const regions = Object.keys(regionsMap).map(regionName => {
      ...
      cmTotalGoal += regTotalGoal
      cmAch += regAch
      cmShb += regShb
      
      return {
        regionName,
        id: \`\${l1.l1_email}-\${regionName}\`,
        sellers,
        blGoal: regBlGoal,
        tlGoal: regTlGoal,
          ...
          <tbody>
            {processedData.map((cm: any) => (
              <React.Fragment key={cm.l1_email}>
                ...
                {expandedCms[cm.l1_email] && cm.regions.map((reg: any) => (
                  <React.Fragment key={reg.id}>
                    ...
                    >
                      ...
                      <td style={{padding:'10px 14px', textAlign:'right'}}>{fmt(reg.blGoal)}</td>
                      <td style={{padding:'10px 14px', textAlign:'right'}}>{fmt(reg.tlGoal)}</td>
                      <td style={{padding:'10px 14px', textAlign:'right', fontWeight: 600}}>{fmt(reg.totalGoal)}</td>
                      <td style={{padding:'10px 14px', textAlign:'right'}}>{fmt(reg.shb)}</td>
                      <td style={{padding:'10px 14px', textAlign:'right', color:'#22C55E', fontWeight: 600}}>{fmt(reg.ach)}</td>
                    </tr>

                    {expandedRegions[reg.id] && reg.sellers.map((s: any) => (
                      <tr key={s.seller_email} style={{borderBottom:'1px dashed rgba(255,255,255,0.02)'}}>
                        <td style={{padding:'6px 14px', paddingLeft: '48px', color: '#6A6258', fontSize: '0.7rem'}}>
                          {s.seller_name}
                        </td>
