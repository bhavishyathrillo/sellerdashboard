...
function getDisplayName(email: string, allSellers?: any[]): string {
  if (!email || !allSellers) return email?.split('@')[0] || 'Unknown'
  ...
  useEffect(() => {
    fetch('/api/admin/hygiene')
      ...
      .then(d => { 
        ...
        setAllSrsSellers(d._srsSellers || [])
        setLoading(false) 
      })
  }, [])
    ...
    <div className={styles.page}>
      ...
      {viewMode === 'cards' && !search.trim() && (
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:'12px'}}>
          {l1Data.map((l1: any) => {
            const tc = l1.total_calls || 0
            const td = l1.total_duration || 0
            const ac = l1.avg_calls_per_seller_per_day || 0
            const ad = l1.avg_duration_per_seller_per_day || 0
            const sc = l1.total_sellers || 0
            ...
            return (
              <div key={l1.l1_email} onClick={() => setSelectedL1(l1)} style={{
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:'0.9rem'}}>{getDisplayName(l1.l1_email, allSrsSellers)}</div>
                    <div style={{fontSize:'0.62rem',color:'#8A8278'}}>{l1.l2_count} L1 Managers · {sc} sellers</div>
                  </div>
                  <div style={{textAlign:'right'}}>
                    <div style={{fontSize:'0.75rem',fontWeight:600,color:'#F4631E'}}>{ac} calls/day</div>
                    <div style={{fontSize:'0.65rem',color:'#22C55E'}}>{fmtDuration(ad)}</div>
                  </div>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'8px'}}>
                  <div style={{background:'rgba(255,255,255,0.02)',borderRadius:'8px',padding:'8px',textAlign:'center'}}>
                    ...
                    <div style={{fontSize:'0.85rem',fontWeight:700,color:'#F4631E'}}><CallIcon />{tc}</div>
                  </div>
                  <div style={{background:'rgba(255,255,255,0.02)',borderRadius:'8px',padding:'8px',textAlign:'center'}}>
                    <div style={{fontSize:'0.55rem',color:'#8A8278',textTransform:'uppercase'}}>Total Duration</div>
                    <div style={{fontSize:'0.85rem',fontWeight:700,color:'#22C55E'}}><ClockIcon />{fmtDuration(td)}</div>
                  </div>
                </div>
                <div style={{height:'3px',background:'rgba(255,255,255,0.06)',borderRadius:'2px'}}>
                  <div style={{height:'100%',background:'linear-gradient(90deg,#F4631E,#C9A84C)',borderRadius:'2px',width:'100%'}}/>
                </div>
              </div>
            )
          })}
      ...
      {selectedL1 && (
        <PopupModal title={`${getDisplayName(selectedL1.l1_email, allSrsSellers)} — Hygiene`} onClose={() => setSelectedL1(null)}>
          {(() => {
            ...
            selectedL1.l2_groups?.forEach((l2: any) => { l2.sellers?.forEach((s: any) => allSellers.push(s)) })
            const dateMap: Record<string, { date: string; call_dials: number; call_duration: number }> = {}
            allSellers.forEach((s: any) => {
              (s.dailyData || []).forEach((d: any) => {
                if (!dateMap[d.date]) dateMap[d.date] = { date: d.date, call_dials: 0, call_duration: 0 }
                dateMap[d.date].call_dials += (d.call_dials || 0)
                dateMap[d.date].call_duration += (d.call_duration || 0)
              })
            })
            const teamDailyData = Object.values(dateMap).sort((a, b) => a.date.localeCompare(b.date))
            const teamTotalCalls = teamDailyData.reduce((s, d) => s + d.call_dials, 0)
            const teamTotalDuration = teamDailyData.reduce((s, d) => s + d.call_duration, 0)
            return (
              <>
                <HygieneChart dailyData={teamDailyData} />
                <div style={{display:'flex',gap:'16px',marginBottom:'12px',fontSize:'0.72rem',color:'#8A8278'}}>
                  <span>Team Total Calls: <strong style={{color:'#F4631E'}}>{teamTotalCalls}</strong></span>
                  <span>Team Total Duration: <strong style={{color:'#22C55E'}}>{fmtDuration(teamTotalDuration)}</strong></span>
                  <span>Avg/Day: <strong>{selectedL1.avg_calls_per_seller_per_day}</strong></span>
                  <span>Sellers: <strong>{allSellers.length}</strong></span>
                </div>
              </>
            )
          })()}
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(250px,1fr))',gap:'10px'}}>
            {selectedL1.l2_groups.map((l2: any) => {
              const l2tc = l2.sellers?.reduce((s:number,r:any)=>s+(r.total_calls||0),0) || 0
              const l2td = l2.sellers?.reduce((s:number,r:any)=>s+(r.total_duration||0),0) || 0
              return (
                <div key={l2.l2_email} onClick={(e) => { e.stopPropagation(); setSelectedL2(l2) }} style={{
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div><div style={{fontWeight:600,fontSize:'0.82rem'}}>{getDisplayName(l2.l2_email, allSrsSellers)}</div><div style={{fontSize:'0.6rem',color:'#8A8278'}}>{l2.seller_count} sellers</div></div>
                    <div style={{textAlign:'right'}}>
                      <div style={{fontSize:'0.7rem',color:'#F4631E'}}><CallIcon />{l2tc}</div>
                      <div style={{fontSize:'0.6rem',color:'#22C55E'}}><ClockIcon />{fmtDuration(l2td)}</div>
                    </div>
                  </div>
                </div>
              )
        ...
        <PopupModal title={`${getDisplayName(selectedL2.l2_email, allSrsSellers)} — Hygiene`} onClose={() => setSelectedL2(null)}>
          <div style={{ marginBottom: '20px', padding: '16px', background: 'rgba(201,168,76,0.05)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: '12px' }}>
            ...
            {(() => {
              ...
              const teamTotalCalls = teamDailyData.reduce((s, d) => s + d.call_dials, 0)
              const teamTotalDuration = teamDailyData.reduce((s, d) => s + d.call_duration, 0)
              if (teamTotalCalls === 0 && teamTotalDuration === 0) return <div style={{ textAlign: 'center', padding: '20px', color: '#8A8278', fontSize: '0.75rem' }}>No call data available for this month</div>
              return (
                <>
                  <HygieneChart dailyData={teamDailyData} />
                  <div style={{ display: 'flex', gap: '16px', fontSize: '0.72rem', color: '#8A8278', marginTop: '8px' }}>
                    <span>Team Total Calls: <strong style={{ color: '#F4631E' }}>{teamTotalCalls}</strong></span>
                    <span>Team Total Duration: <strong style={{ color: '#22C55E' }}>{fmtDuration(teamTotalDuration)}</strong></span>
                    <span>Sellers: <strong>{allSellers.length}</strong></span>
                  </div>
          ...
          <h4 style={{ fontSize: '0.8rem', fontWeight: 700, color: '#C9A84C', marginBottom: '10px' }}>
            👥 All Sellers ({(selectedL2.sellers || []).length + 1})
          </h4>
          {(() => {
            ...
            return (
              <div onClick={() => setSelectedSeller(l2SellerData)} style={{ background: 'rgba(244,99,30,0.08)', border: '1px solid rgba(244,99,30,0.2)', borderRadius: '10px', padding: '12px', marginBottom: '6px', cursor: 'pointer' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontWeight: 600, fontSize: '0.8rem' }}>{getDisplayName(selectedL2.l2_email, allSrsSellers)} <span style={{ fontSize: '0.6rem', color: '#F4631E', marginLeft: '6px' }}>(Manager)</span></span>
                  <div style={{ display: 'flex', gap: '14px', fontSize: '0.72rem' }}>
                    <span style={{ color: '#F4631E' }}><CallIcon />{l2SellerData.total_calls || 0}</span>
                    <span style={{ color: '#22C55E' }}><ClockIcon />{fmtDuration(l2SellerData.total_duration || 0)}</span>
                  </div>
                </div>
              </div>
            )
          })()}
          {(selectedL2.sellers || []).filter((s: any) => s.seller_email?.toLowerCase() !== selectedL2.l2_email?.toLowerCase()).map((s: any) => (
            <div key={s.seller_email} onClick={() => setSelectedSeller(s)} style={{ background: '#141414', border: '1px solid #232323', borderRadius: '10px', padding: '12px', marginBottom: '6px', cursor: 'pointer' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontWeight: 600, fontSize: '0.8rem' }}>{getDisplayName(s.seller_email, allSrsSellers)}</span>
                <div style={{ display: 'flex', gap: '14px', fontSize: '0.72rem' }}>
