...
export default function AdminHygienePage() {
  ...
  useEffect(() => {
    fetch('/api/admin/hygiene')
      .then(r => r.json())
      .then(d => { setL1Data(d.l1_data || []); setMonthName(d.month || ''); setLoading(false) })
  }, [])
      ...
      l1.l2_groups?.forEach((l2: any) => {
        ...
        l2.sellers?.forEach((s: any) => {
          ...
          if (l1Match || l2Match || sMatch) {
            s.dailyData?.forEach((d: any) => { results.push({ ...d, seller_name: s.seller_name, l1_name: l1.l1_name, l2_name: l2.l2_name }) })
          }
        })
      })
    })
  ...
  useEffect(() => { if (selectedL1?.dailyData) renderChart(selectedL1.dailyData); return () => { if (chartInstance.current) chartInstance.current.destroy() } }, [selectedL1])

  if (loading) return <div className={styles.loading}><div className={styles.spinner}/><p>Loading...</p></div>

  const totalCalls = l1Data.reduce((s: number, l: any) => s + l.total_calls, 0)
  const totalDuration = l1Data.reduce((s: number, l: any) => s + l.total_duration, 0)
  const totalSellers = l1Data.reduce((s: number, l: any) => s + l.l2_groups?.reduce((ss: number, l2: any) => ss + (l2.seller_count || 0), 0), 0)
  const avgCallsPerDay = totalSellers > 0 ? Math.round(totalCalls / totalSellers) : 0
  const avgDurationPerDay = totalSellers > 0 ? Math.round(totalDuration / totalSellers) : 0
    ...
    <div className={styles.page}>
      ...
      <div className={styles.kpiGrid}>
        <div className={`${styles.kpiCard} ${styles.kpiPrimary}`}>
          ...
          <div className={styles.kpiValue}>{totalCalls}</div><div className={styles.kpiLabel}>Total Calls</div><div className={styles.kpiTrend}>{monthName}</div>
        </div>
        <div className={styles.kpiCard}>
          ...
          <div className={styles.kpiValue}>{avgCallsPerDay}</div><div className={styles.kpiLabel}>Avg Calls/Day</div><div className={styles.kpiTrend}>Per seller</div>
        </div>
        <div className={styles.kpiCard}>
          ...
          <div className={styles.kpiValue}>{fmtDuration(avgDurationPerDay)}</div><div className={styles.kpiLabel}>Avg Duration/Day</div><div className={styles.kpiTrend}>Per seller</div>
        </div>
      </div>
      ...
      {/* CARDS VIEW - L1 KPI Cards with daily avg */}
      {viewMode === 'cards' && !search.trim() && (
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:'12px'}}>
          {l1Data.map((l1: any) => {
            const sellerCount = l1.l2_groups?.reduce((s: number, l2: any) => s + (l2.seller_count || 0), 0) || 1
            const avgCalls = Math.round(l1.total_calls / sellerCount)
            const avgDur = Math.round(l1.total_duration / sellerCount)
            return (
              <div key={l1.l1_email} onClick={() => { setSelectedL1(l1); setTimeout(() => renderChart(l1.dailyData), 200) }} style={{
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}>
                  <div><div style={{fontWeight:700,fontSize:'0.9rem'}}>{l1.l1_name}</div><div style={{fontSize:'0.62rem',color:'#8A8278'}}>{l1.l2_count} L1 Managers · {sellerCount} sellers</div></div>
                </div>
                {/* Daily Average Stats */}
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'8px'}}>
                  <div style={{background:'rgba(255,255,255,0.02)',borderRadius:'8px',padding:'8px',textAlign:'center'}}>
                    ...
                    <div style={{fontSize:'0.85rem',fontWeight:700,color:'#F4631E'}}><CallIcon />{l1.total_calls}</div>
                  </div>
                  <div style={{background:'rgba(255,255,255,0.02)',borderRadius:'8px',padding:'8px',textAlign:'center'}}>
                    <div style={{fontSize:'0.55rem',color:'#8A8278',textTransform:'uppercase'}}>Total Duration</div>
                    <div style={{fontSize:'0.85rem',fontWeight:700,color:'#22C55E'}}><ClockIcon />{fmtDuration(l1.total_duration)}</div>
                  </div>
                  <div style={{background:'rgba(255,255,255,0.02)',borderRadius:'8px',padding:'8px',textAlign:'center'}}>
                ...
                <div style={{height:'3px',background:'rgba(255,255,255,0.06)',borderRadius:'2px'}}><div style={{height:'100%',background:'linear-gradient(90deg,#F4631E,#C9A84C)',borderRadius:'2px',width:`${Math.min((l1.total_calls/Math.max(totalCalls,1))*100,100)}%`}}/></div>
              </div>
            )
          })}
        </div>
      ...
      {selectedL1 && (
        <PopupModal title={`${selectedL1.l1_name} — Hygiene`} onClose={() => setSelectedL1(null)}>
          ...
          <div style={{display:'flex',gap:'12px',marginBottom:'12px',fontSize:'0.7rem',color:'#8A8278'}}><span>Total Calls: <strong style={{color:'#F4631E'}}>{selectedL1.total_calls}</strong></span><span>Total Duration: <strong style={{color:'#22C55E'}}>{fmtDuration(selectedL1.total_duration)}</strong></span></div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(250px,1fr))',gap:'10px'}}>
            {selectedL1.l2_groups.map((l2: any) => (
              <div key={l2.l2_email} onClick={(e) => { e.stopPropagation(); setSelectedL2(l2) }} style={{background:'#141414',border:'1px solid #232323',borderRadius:'12px',padding:'14px',cursor:'pointer'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{fontWeight:600,fontSize:'0.82rem'}}>{l2.l2_name}</div><div style={{fontSize:'0.6rem',color:'#8A8278'}}>{l2.seller_count} sellers</div></div><div style={{textAlign:'right'}}><div style={{fontSize:'0.7rem',color:'#F4631E'}}><CallIcon />{l2.total_calls}</div><div style={{fontSize:'0.6rem',color:'#22C55E'}}><ClockIcon />{fmtDuration(l2.total_duration)}</div></div></div>
              </div>
            ))}
          </div>
        </PopupModal>
        ...
        <PopupModal title={`${selectedL2.l2_name} — Sellers`} onClose={() => setSelectedL2(null)}>
          {selectedL2.sellers.map((s: any) => (
            <div key={s.seller_email} onClick={() => setSelectedSeller(s)} style={{background:'#141414',border:'1px solid #232323',borderRadius:'10px',padding:'12px',marginBottom:'6px',cursor:'pointer'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontWeight:600,fontSize:'0.8rem'}}>{s.seller_name}</span><div style={{display:'flex',gap:'14px',fontSize:'0.72rem'}}><span style={{color:'#F4631E'}}><CallIcon />{s.total_calls}</span><span style={{color:'#22C55E'}}><ClockIcon />{fmtDuration(s.total_duration)}</span></div></div>
            </div>
          ))}
        </PopupModal>
      )}
