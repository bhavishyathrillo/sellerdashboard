    ...
    <div className={styles.kpiGrid}>
      ...
      <div className={`${styles.kpiCard} ${metrics.mishandledPct > 20 ? styles.kpiDanger : styles.kpiSuccess}`}>
        ...
        <div className={styles.kpiSplit}>
          <div className={styles.kpiSplitCol}>
            <span className={styles.kpiSplitLabel}>Mishandled %</span>
            <span className={styles.kpiSplitValue} style={{ color: metrics.mishandledPct > 20 ? '#EF4444' : '#22C55E' }}>
              {metrics.mishandledPct}%
            </span>
          </div>
          <div className={styles.kpiSplitCol}>
            <span className={styles.kpiSplitLabel}>Mishandled Count</span>
            <span className={styles.kpiSplitValue} style={{ color: metrics.mishandledPct > 20 ? '#EF4444' : '#22C55E' }}>
              {metrics.mishandledLeads}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className={styles.kpiCard}>
        ...
        <div className={styles.kpiSplit}>
          <div className={styles.kpiSplitCol}>
            <span className={styles.kpiSplitLabel}>Conv Happened</span>
            <span className={styles.kpiSplitValue} style={{ color: '#22C55E' }}>
              {metrics.conversationHappenedLeads ?? 0}
            </span>
            <span className={styles.kpiSub}>{convoPct}%</span>
          </div>
          <div className={styles.kpiSplitCol}>
            <span className={styles.kpiSplitLabel}>2 Attempts Done</span>
            <span className={styles.kpiSplitValue} style={{ color: '#C9A84C' }}>
              {metrics.twoAttemptsDoneLeads ?? 0}
            </span>
            <span className={styles.kpiSub}>{attemptsPct}%</span>
          </div>
        </div>
      </div>
    </div>
...
function LeadsTable({ leads }: { leads: PriorityLead[] }) {
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px', color: '#8A8278' }}>{search ? 'No leads match your search' : 'No priority leads found'}</div>
      ) : (
...
// Card header — Lead count (orange bold) + Mishandled % pill
...
export default function PriorityPage({ session }: Props) {
    ...
    const filteredL1Groups = search.trim()
      ? l1Groups.filter((l1: any) => {
          ...
          return l1.l2_groups?.some((l2: any) => {
            ...
            return l2.sellers.some((s: any) => s.seller_name?.toLowerCase().includes(search.toLowerCase()) || s.seller_email?.toLowerCase().includes(search.toLowerCase()))
          })
        })
      : l1Groups
      ...
      <div className={styles.page}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          <div>
            ...
            <p style={{ fontSize: '0.72rem', color: '#8A8278', marginTop: '2px' }}>{l1Groups.length} Category Managers · {data.totalSellers} sellers</p>
        ...
        {tableView ? (
          <div style={{ textAlign: 'center', padding: '40px', color: '#8A8278' }}>Table view coming soon</div>
        ) : (
                            ...
                            <div onClick={(e) => { e.stopPropagation(); toggleL2(l2.l2_email) }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', cursor: 'pointer' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                <div>
                                  ...
                                  <span style={{ fontSize: '0.62rem', color: '#8A8278' }}>{l2.seller_count} sellers</span>
                                </div>
                              </div>
  ...
  if (isL1 && data?.isL1) {
    ...
    const filteredL2Groups = search.trim()
      ? l2Groups.filter((g: any) => g.l2_name?.toLowerCase().includes(search.toLowerCase()) || g.sellers.some((s: any) => s.seller_name?.toLowerCase().includes(search.toLowerCase()) || s.seller_email?.toLowerCase().includes(search.toLowerCase())))
      : l2Groups
    ...
    return (
      <div className={styles.page}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          <div>
            <h1 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#C9A84C' }}>Priority Leads</h1>
            <p style={{ fontSize: '0.72rem', color: '#8A8278', marginTop: '2px' }}>{l2Groups.length} L1 Managers · {data.totalSellers} sellers</p>
            <p style={{ fontSize: '0.6rem', color: '#5A5650', marginTop: '2px' }}>Updates every 40 min</p>
          </div>
          <div style={{ display: 'flex', gap: '3px', background: '#141414', border: '1px solid #232323', borderRadius: '8px', padding: '3px' }}>
            <button onClick={() => setTableView(false)} style={{ padding: '7px 16px', border: 'none', borderRadius: '6px', background: !tableView ? 'rgba(244,99,30,0.15)' : 'transparent', color: !tableView ? '#F4631E' : '#8A8278', cursor: 'pointer', fontSize: '0.72rem', fontWeight: 600 }}>Cards</button>
                  ...
                  <div onClick={() => toggleL2(group.l2_email)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', cursor: 'pointer', userSelect: 'none' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div>
                        ...
                        <span style={{ fontSize: '0.62rem', color: '#8A8278' }}>{group.seller_count} sellers</span>
                      </div>
                    </div>
    ...
    return (
      <div className={styles.page}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
          <div>
            <h1 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#C9A84C' }}>Priority Leads</h1>
            <p style={{ fontSize: '0.72rem', color: '#8A8278', marginTop: '2px' }}>{sellers.length} seller{sellers.length !== 1 ? 's' : ''} · Team View</p>
            <p style={{ fontSize: '0.6rem', color: '#5A5650', marginTop: '2px' }}>Updates every 40 min</p>
          </div>
