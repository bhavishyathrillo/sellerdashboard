...
export default function LeaderboardPage({ session }: Props) {
  ...
  if (loading) return (
    <div className={styles.loaderWrap}>
      ...
      <p className={styles.loaderText}>Summoning the champions...</p>
    </div>
  )
    ...
    <div className={styles.page}>
      ...
      <div className={styles.filterBar}>
        <select value={haul} onChange={e => setHaul(e.target.value)} className={styles.filter}>
          ...
          <option value="Short Haul">✈️ Short Haul</option>
        ...
        <select value={region} onChange={e => setRegion(e.target.value)} className={styles.filter}>
          ...
          {[...new Set(sellers.map(s => s.region).filter(Boolean))].map(r => (
      ...
      <div className={styles.podiumSection}>
        <div className={styles.podium}>
          {podiumData.map((s, i) => {
            const pos = rankOrder[i]
            const isGold = pos === 0
            return (
              <div
                key={s?.seller_email || i}
                className={`${styles.podiumCard} ${isGold ? styles.cardGold : pos===1 ? styles.cardSilver : styles.cardBronze}`}
                style={{ animationDelay: `${i*0.2}s` }}
              >
                {isGold && <div className={styles.crown}>👑</div>}
                {isGold && <div className={styles.sparkles}>✨</div>}
                <div className={styles.rankNum} style={{color:glowColors[pos]}}>#{pos+1}</div>
                <div className={`${styles.avatarWarp} ${isGold ? styles.avatarGold : ''}`}>
                  <div className={styles.avatarInner}>{initials(s?.seller_name)}</div>
                  <div className={styles.avatarRing} style={{borderColor:glowColors[pos]}} />
                </div>
                <div className={styles.podName}>{s?.seller_name}</div>
                <div className={styles.podTeam}>{s?.l1_name}</div>
                <div className={styles.podProgress}>
                  <div className={styles.podProgressBar}>
                    <div className={styles.podProgressFill} style={{
                      width: `${Math.min(s?.goal_achieved_percent||0,100)}%`,
                      background: `linear-gradient(90deg, ${glowColors[pos]}, #F4631E)`
                    }} />
                  </div>
                </div>
                <div className={styles.podStats}>
                  <span className={styles.podPct} style={{color:glowColors[pos]}}>{s?.goal_achieved_percent?.toFixed(1)}%</span>
                  <span className={styles.podAmt}>{fmt(s?.actual_achieved_monthly||0)}</span>
                </div>
      ...
      <div className={styles.haulRow}>
        {hauls.map(h => {
          ...
          return (
            <div key={h} className={styles.haulBox}>
              <h3 className={styles.haulBoxTitle}>{h==='Short Haul'?'✈️':h==='Long Haul'?'🌏':'🇮🇳'} {h}</h3>
              {hSellers.map((s,i) => (
                <div key={s.seller_email} className={styles.haulLine}>
                  <span>{['🥇','🥈','🥉'][i]}</span>
                  <span className={styles.haulLineName}>{s.seller_name}</span>
                  <span className={styles.haulLinePct}>{s.goal_achieved_percent?.toFixed(1)}%</span>
                </div>
              ))}
            </div>
          )
      ...
      <div className={styles.rankingsSection}>
            ...
            return (
              <div key={s.seller_email} className={styles.rankingRow}>
                ...
                <div className={styles.rankingInfo}>
                  ...
                  <span className={styles.rankingMeta}>{s.l1_name} · {s.haul}</span>
                </div>
                <div className={styles.rankingBar}>
                  <div className={styles.rankingBarFill} style={{width:`${Math.min(s.goal_achieved_percent||0,100)}%`}} />
                </div>
        ...
        {sellers.length > 10 && (
          <button onClick={() => setShowAll(!showAll)} className={styles.moreBtn}>
