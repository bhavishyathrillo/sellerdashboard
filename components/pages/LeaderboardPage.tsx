...
export default function LeaderboardPage({ session }: Props) {
  ...
  // Load all regions once on mount
  useEffect(() => {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (res.ok) {
        setSellers(data || [])
        // Update regions only on initial load (no filters)
        if (!haul && !region) {
          setAllRegions([...new Set((data || []).map((s: Seller) => s.region).filter(Boolean))].sort())
        }
      }
              ...
              <div key={s?.seller_email || i} className={`${styles.podiumCard} ${isGold ? styles.cardGold : pos===1 ? styles.cardSilver : styles.cardBronze}`} style={{ animationDelay: `${i*0.2}s` }}>
                ...
                <div className={styles.podProgress}>
                  <div className={styles.podProgressBar}>
                    <div className={styles.podProgressFill} style={{ width: `${Math.min(s?.goal_achieved_percent||0,100)}%`, background: `linear-gradient(90deg, ${glowColors[pos]}, #F4631E)` }} />
                  </div>
                </div>
