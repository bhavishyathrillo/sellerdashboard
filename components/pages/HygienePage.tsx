            ...
            <svg viewBox={`0 0 ${chartData.length * 50} 120`} className={styles.svg}>
              ...
              {chartData.map((d, i) => (
                <g key={i}>
                  ...
                  {i<chartData.length-1&&<line x1={i*50+25} y1={115-((d.call_dials||0)/maxDials)*90} x2={(i+1)*50+25} y2={115-((chartData[i+1]?.call_dials||0)/maxDials)*90} stroke="#F4631E" strokeWidth="2.5" strokeLinecap="round"/>}
                </g>
              ))}
            </svg>
            {tooltip.show&&<div className={styles.tooltip} style={{left:tooltip.x,top:tooltip.y}}><span className={styles.tooltipDate}>{new Date(tooltip.date).toLocaleDateString('en-IN',{weekday:'short',day:'numeric',month:'short'})}</span><span className={styles.tooltipVal}>{tooltip.value} calls</span></div>}
            <div className={styles.chartXAxis}>{chartData.map((d,i)=><span key={i} className={styles.xLabel}>{new Date(d.date).getDate()}</span>)}</div>
          </div>
        </div>
      </div>
      ...
      <div className={styles.tableCard}>
        ...
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            <thead><tr><th>Date</th><th>Calls</th><th>Duration</th><th>Status</th></tr></thead>
            <tbody>
              {last14Days.map((row, i) => {
                ...
                return (
                  <tr key={i} className={met?styles.rowGood:mins>0?styles.rowBad:''}>
                    <td className={styles.dateCell}><span className={styles.dateDay}>{new Date(row.date).toLocaleDateString('en-IN',{weekday:'short'})}</span><span className={styles.dateFull}>{new Date(row.date).toLocaleDateString('en-IN',{day:'numeric',month:'short'})}</span></td>
                    <td className={styles.callsCell}>{row.call_dials||'—'}</td>
                    <td>{mins>0?`${Math.floor(mins/60)}h ${mins%60}m`:'—'}</td>
                    <td>{met?<span className={styles.badgeGreen}>✅ Met</span>:mins>0?<span className={styles.badgeRed}>❌ Missed</span>:<span className={styles.badgeNone}>—</span>}</td>
                  </tr>
                )
              })}
            </tbody>
